...
const addMessageToChat = async (req, res) => {
    try {
        ...
        const messageSaved = await newMsg.save()
        chat.messages.push(messageSaved)

        const messageAdded = await chat.save()
        if (!chat) return res.status(404).json({ "message": "Chat not found" })
        console.log("here is message saved",messageSaved)
...
export { getMyOwnChats, addMessageToChat, getChatById }